...
Node A implementation, evidence preservation, and local code review are complete, but runtime M0 is not accepted. The fixture, independent package/workflow gates, exact result and diagnostic validators, bounded launcher, earliest-Main lifecycle evidence, finite terminal redaction, and narrowly reviewed Linux runner launch policy are ready; local fixture tests pass 31/31, Desktop direct typecheck and fixture lint pass, and the isolated bundle builds. The prior hosted runs remain failed: Linux rejected cancellation, RSS, and real-plugin/dependency behavior, while Windows returned from the packaged launcher without a result. The next hosted run must retain both Linux modes and the Windows diagnostic/lifecycle evidence. Production routing remains unchanged until corrected Linux development/packaged and Windows packaged results pass. If the custom-scheme behavior itself fails after the allowed evidence-based correction, this plan stops with measurements; Plan B is a separate local HTTP/2-over-TLS decision, never private process framing.
