...
- **Implementation:** Platform resolution selects a new `states.ios.tsx` implementation for all existing imports. A small shared contract keeps the public props consistent with the React Native implementation. The native route state uses SwiftUI primitives so retry actions and pending progress can stay in one coherent layout. The original iOS 16 compatibility rationale was superseded when Mobile moved to an iOS 26 minimum.
