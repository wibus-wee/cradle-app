...
describe('browser-use protocol framing', () => {
  it('decodes a complete frame', () => {
    ...
    const command: BrowserCommand = { id: 'one', ownerId: 'chat:session-a', type: 'tabs_list' }
  ...
  it('decodes multiple frames in one chunk', () => {
    const decoder = new FrameDecoder()
    const first: BrowserCommand = { id: 'first', ownerId: 'chat:session-a', type: 'tabs_list' }
    const second: BrowserCommand = {
      id: 'second',
      ownerId: 'chat:session-a',
      type: 'keyboard',
      key: 'Enter',
