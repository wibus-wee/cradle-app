...
export interface CodexAccountDiagnosticsPanelViewProps {
  diagnostics: CodexAccountDiagnostics | null
  diagnosticsLoading: boolean
  diagnosticsError: unknown
  whamDiagnostics: CodexWhamDiagnostics | null
  whamLoading: boolean
  ...
  onCancelResetCredit: () => void
  onConfirmResetCredit: () => void
}

export function CodexAccountDiagnosticsPanelView({
  diagnostics,
  diagnosticsLoading,
  diagnosticsError,
  whamDiagnostics,
  ...
  onUseResetCredit,
  onCancelResetCredit,
  onConfirmResetCredit,
}: CodexAccountDiagnosticsPanelViewProps) {
  ...
  const whamResetCredits = whamResetCreditResult?.ok
    ...
    : null
  const statusKind = deriveStatusKind(diagnostics, diagnosticsError)
  ...
  return (
    <>
      <SummaryCard
        ...
        onRefresh={onRefresh}
        onOpen={() => onDialogOpenChange(true)}
      />
      ...
      <Dialog open={dialogOpen} onOpenChange={onDialogOpenChange}>
        ...
        >
          <DialogHeaderBar
            diagnostics={diagnostics}
            loading={diagnosticsLoading || whamLoading}
            error={diagnosticsError}
            onRefresh={onRefresh}
            canUseResetCredit={canUseResetCredit}
            resetCreditCount={resetCreditCount}
            onUseResetCredit={onUseResetCredit}
          ...
          <Tabs value={activeTab} onValueChange={onActiveTabChange} className="flex min-h-0 flex-1 flex-col gap-0">
            <div className="flex items-center justify-between gap-2 px-6 pt-1">
              <TabsList variant="line" className="mb-2 h-9 px-1">
            ...
            <TabsContent value="account" className="mt-0 min-h-0 flex-1 overflow-hidden">
              <ScrollArea className="h-full">
                <AccountTab diagnostics={diagnostics} loading={diagnosticsLoading} error={diagnosticsError} loadingLabel={loadingLabel} />
              </ScrollArea>
            </TabsContent>
            ...
            <TabsContent value="usage" className="mt-0 min-h-0 flex-1 overflow-hidden">
              <ScrollArea className="h-full">
                <UsageTab diagnostics={diagnostics} loading={diagnosticsLoading} error={diagnosticsError} loadingLabel={loadingLabel} />
              </ScrollArea>
            </TabsContent>
          </Tabs>
        </DialogContent>
      ...
      <AlertDialog open={resetDialogOpen} onOpenChange={onResetDialogOpenChange}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel size="sm" onClick={onCancelResetCredit}>Cancel</AlertDialogCancel>
            <AlertDialogAction size="sm" onClick={onConfirmResetCredit}>
              Use credit
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
...
function renderNoticeBody(
  ...
  if (error) {
    return (
      <div className="flex items-start gap-2.5 rounded-xl bg-destructive/6 px-3.5 py-3 text-destructive ring-1 ring-destructive/10">
        <AlertTriangleIcon className="mt-0.5 !size-3.5 shrink-0 !text-destructive" />
        <div className="min-w-0">
          <div className="text-[12px] font-medium">Couldn’t load account diagnostics</div>
...
function buildHeaderMetaLine({
  ...
  if (error) {
    parts.push(`Refresh failed: ${apiErrorMessage(error)}`)
  }
