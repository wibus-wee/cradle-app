...
- **agent-runtime-settings.tsx**: 统一 Agent Profile 管理界面；Provider 列表展示 Cradle-owned manual provider profiles 与 external runtime target records，由 TanStack Query owner 驱动，壳层只保留选中/过滤 UI 状态；manual provider 支持单项编辑 / 删除 / 启停、多选批量操作以及列表内 `Cmd/Ctrl+A`、`Escape`、`Delete/Backspace` 快捷键和 `Shift+click` 连续区间选择，external record 打开 external record 详情；新增 / 导入 manual provider 后立即触发 provider model cache warm，不等用户打开模型面板；Settings Providers 首屏在 profiles 与 external source/record queries 成功后记录 performance gate；列表行副标题展示 endpoint host（行内不再重复 provider kind 标签），checkbox 仅 hover / 已选 / 选择模式时显示，连接状态圆点位于行尾，disabled profile 以透明度弱化，行首品牌图标统一使用 ProviderIconTile；新增 provider 走 AddProviderDialog 模态两步流程，detail pane 只承载详情 / 空状态，统一内边距并以 keyed crossfade 过渡，避免切换时布局跳动
- **add-provider-dialog.tsx**: 「新增 Provider」模态框壳，固定高度承载 ProviderSetupFlow 两步流程，关闭时重置预设选择；替代原先在 detail pane 内嵌模板选择器 / 表单的交互
- **provider-setup-flow.tsx**: 新增 Provider 的两步流程组件（可搜索模板画廊 ⇄ 凭证表单，滑动步进过渡）；画廊含 Featured 大卡片区（anthropic / openai / universal）与品牌图标行列表；dialog-agnostic，Settings 模态框与 onboarding 首启向导共用
- **provider-setup-form.tsx**: 预设凭证表单（RHF owner），盒式呈现语言（`SetupField` 堆叠字段 + `AuthModeSegmented` 分段选择器 + `ui/Input` / `ui/Button`），auth mode 字段组 crossfade 切换，底部固定 footer 内联状态信息；同时导出 `SetupField` / `AuthModeSegmented` / `InfoCallout` 供 profile 详情面板复用；承载 secret 创建、provider inventory cache warm 与保存后即时连接测试；preset metadata 不得写入 custom models
...
- **import-provider-dialog.tsx**: 手动 Import Provider 弹窗，调用配置片段 parser 自动识别 API key 与 endpoint，并以每个 provider 自己的解码后 apiKey 创建 credential，避免展示解码值但保存编码值；支持打开时自动读取剪贴板预填、拖入或选择 `.env` / JSON 配置文件、与现有 provider 的 endpoint 冲突标记、一键扫描本机 Agent 工具配置（`POST /external-provider-sources/local-scan`），导入完成后自动触发连接测试并回写状态缓存
...
- **provider-settings-utils.ts**: Provider settings 的共享常量与纯函数，承载 provider kind label、profile id 构造与 preset 匹配逻辑
...
- **models-panel.tsx**: Provider 模型可见性面板，复用 Agent Runtime 的模型可见性语义；模型行以单行 muted 元信息文本（context window、reasoning / tools / image input、models.dev 映射 id）替代徽章墙，仅 registry unmatched 状态以 amber 文本提示，并支持按 Available Model 行保存全局 registry 映射或手工 registry 条目；完全没有本地模型缓存时由 manual profile detail 或 CC-Switch external record detail 主动发起一次 Fetch Models
- **provider-connection-test.tsx**: Provider 连接测试 UI，封装 `POST/GET /provider-targets/:id/test` 的查询与变更；详情面板提供 Test / Deep test 按钮与结构化状态徽章（ok / auth_failed / network_error / endpoint_error / model_unavailable + 延迟），列表行展示最近一次缓存测试的状态圆点
- **use-provider-presets.ts**: 服务端 Provider 预设目录（`GET /provider-presets`，models.dev + 本地 overlay）与本地三个 wizard 预设的合并 hook；服务端预设只贡献连接、认证与展示 metadata，模型 inventory 始终从已配置 provider target 动态发现，加载失败时回退到本地预设
- **profile-detail-panel.tsx**: Manual provider 详情面板，继续以 RHF 作为表单 owner，并把模型缓存读取 / 手动 inventory refresh / registry 映射 / Claude model matrix provider default / 自动保存 / 删除确认等瞬时 UI 状态收口到局部 reducer，避免细碎 `useState` 级联；连接测试由 provider-connection-test.tsx 独立承载；模型 registry 映射通过全局 model registry API 保存，custom models 通过 provider-target API 保存，以便和 external runtime target 使用同一语义
