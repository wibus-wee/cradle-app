...
- **browser-annotation-adjustment-panel-view.tsx**: Fixture-driven Design/CSS inspector View (Position / Layout / Dimensions / Spacing / Typography / Fill / Stroke / Effects). Storybook: `App/Browser/Annotation Adjustment Panel`.
- **browser-annotation-adjustment-panel.tsx**: Container that wires `annotationAdjustmentSession` from the BrowserPanel store into the View and dispatches `browser:annotation-adjustment-apply`.
- **browser-annotation-design-fields.ts**: Field model for the Design inspector (groups, segment options, scrub helpers). Mirrors the desktop preload design CSS property list.
- **fixtures/browser-annotation-adjustment.ts**: Selected-element + draft-change fixtures for Storybook.
...
- **subagent-output-panel.tsx**: Browser Panel subagent tab content; reads provider-native thread metadata and turn history from Chat Runtime, stores projected messages in the renderer-only live chat store under a synthetic provider-thread view id, subscribes to provider-thread AI SDK chunk SSE, and renders the standard Chat transcript `MessageBubbleView` surface without implementing a separate chat renderer.
...
- **use-local-servers.ts**: Electron local-server discovery hook used by Browser Panel for blank-tab suggestions.
