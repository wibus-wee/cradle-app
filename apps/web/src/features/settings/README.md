...
- **data-backup-settings.tsx / data-backup-settings-view.tsx / data-backup-settings-view.stories.tsx**: Backup & Restore 设置页的 Electron adapter、fixture-driven View 与 ready/exported/confirm/failure fixtures。Adapter 只负责 native file picker、操作状态读取和 restart scheduling；View 只接收 typed copy/state/callbacks。导出与恢复的 filesystem、checksum、health-check 和 rollback 语义由 Desktop `data-backup.ts` 拥有，renderer 不直接读写 archive。
