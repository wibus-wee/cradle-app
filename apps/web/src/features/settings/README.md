...
- **about-settings.tsx**: 关于设置页，使用紧凑 Settings row 布局说明 Cradle-owned Application Support 数据目录，明确列出用户确认或显式操作后可能写入 Application Support 外的 workspace、skills、provider-native skill roots 与 CLI command 路径，并提供默认开启、可随时关闭的匿名产品分析开关。
...
- **data-backup-settings.tsx / data-backup-settings-view.tsx / data-backup-settings-view.stories.tsx**: Backup & Restore 设置页的 Electron adapter、fixture-driven View 与 ready/exported/confirm/failure fixtures。Adapter 只负责 native file picker、操作状态读取、已导出 archive 的系统文件管理器定位和 restart scheduling；View 只接收 typed copy/state/callbacks。导出与恢复的 filesystem、checksum、health-check 和 rollback 语义由 Desktop `data-backup.ts` 拥有，renderer 不直接读写 archive。
...
- **feature-settings.tsx**: Features 设置页，编辑 server-owned App feature flags，包括默认关闭的 Turn Checkpoints 与是否允许 Cradle 写入 Codex/Claude provider-native skill roots；前端入口只读取这些 flags，不拥有门控语义。
...
- **network-settings.tsx**: 导出 `ServerAccessSettingsGroup` 与 `ProxySettingsGroup`，嵌入 Network 设置页（与 Server Endpoint 合并）。Network 只负责 Electron-managed Cradle Server API 的监听范围与 outbound proxy；Fabric 主机、设备访问和公网地址由 `features/nodes/fabric-settings.tsx` 在 Devices & Fabric 页面拥有，避免用户在 Network 设置中理解 relayd 实现细节。
...
- **shortcut-settings.tsx**: Shortcuts 设置页，承载 desktop-owned AppShot bare modifier 全局触发键（Double Command / Double Option / Double Shift）和启停开关；保存后复用 desktop preferences API，并在 Electron 中通过 `native.setDesktopPreferences` 立即同步 mac bridge input configuration。页面下方使用现有 Settings row primitive 只读列举 Cradle 内置 application/contextual 快捷键，Command-style action 同时列出 Windows/Linux 的 Control 变体，不新增 shortcut storage ownership。
