...
import { readPluginContextPart, readSkillContextPart } from '../context/chat-context-parts'
 ...
 */
export function readUserMessageDraft(message: UIMessage | undefined | null): UserMessageDraft | null {
  ...
  const text = textSegments.map(segment => segment).join('\n').trim()

  if (!text && contextParts.length === 0 && files.length === 0) {
    return null
  }

  return { text, contextParts, files }
}
