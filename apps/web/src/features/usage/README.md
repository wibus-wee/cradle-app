...
- **usage-fleet.ts**: Fleet usage model (this device + reachable Fabric nodes, each with its own daily series). Merging across devices is a renderer concern — the server never aggregates cross-device.
- **use-fleet-usage.ts**: Loads each online node's Usage API via the Fabric upstream proxy (`fetchNodeUpstreamJson`) and merges with local series; returns `null` when no Fabric nodes exist so the dashboard keeps its single-device surface. Offline nodes and failed reads surface as `unavailable` devices.
