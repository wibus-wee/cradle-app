...
- **use-fleet-usage.ts**: Loads each online node's full Usage API via the Fabric upstream proxy (`fetchNodeUpstreamJson`) and merges with local series; returns `null` when no Fabric nodes exist so the dashboard keeps its single-device surface. Offline nodes and failed reads surface as `unavailable` devices; loading nodes simply join `merged` progressively.
