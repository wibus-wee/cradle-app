...
- **use-issue-execution-association.ts**: Issue–execution association reconciler — consumes the server workflow's typed `{ participantKind, participantId, previousIssueId, nextIssueId }` transitions and refreshes the participant projection (Session detail/list/runtime/queue through the Plan 050 `session-projection` gateway, Session Group through `workspace/session-group-projection`), the participant's linked-issue ref, and both old and new Issue linked Session/Group lists. `useLinkIssue`/`useUnlinkIssue` run the same reconciler on success and re-reconcile touched projections on failure so caches converge to the server-confirmed state.
- **use-issue-execution-association.test.ts**: Real-`QueryClient` coverage of link/unlink/relink reconciliation, session-group participants, and failure rollback across participant and old/new Issue projections.
- **use-kanban.ts**: Kanban board 与 Issue-owned status、milestone、relation、comment、Activity projection、raw field-change history、bulk update、label patching、delegation 操作的 TanStack Query hooks；高影响 workspace 读取使用共享 query refresh policy，使外部 CLI、agent 和多窗口变更无需整页刷新即可可见。Association mutations live in `use-issue-execution-association.ts`; this module keeps the `useLinkedIssue` read and the `kanbanKeys.linkedIssue*` key family it shares with the reconciler.
