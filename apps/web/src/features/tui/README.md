...
- **shell-view.tsx**: ShellView component — one retained xterm runtime per session. Visibility changes disable stdin, clear focus/selection while hidden, and fit/focus on reveal without stopping the PTY. If the backing shell was reaped (lease expiry after disconnect, failed first measure, or `terminal_not_running`), the view re-issues `shell/start` on reveal, pointer-down, or input rather than staying silent until a new tab is opened; unmount cleanup remains explicit and owner-scoped.
