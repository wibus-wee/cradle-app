...
Source-dirty and creation failures stay in context. Work starts from the
current workspace HEAD by default; the compact base-branch control can pass an
explicit local or remote `baseBranch` (for example `origin/main`) so local WIP
is left untouched. Starting Work authorizes local isolated execution only and
never grants automatic GitHub delivery.
