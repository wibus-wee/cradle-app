...
- **use-global-event-listeners.ts**: App-shell side-effect hook wiring PTY pushes, keyboard shortcuts, visible-chat ownership, and chat run-settled unread reconciliation; BrowserPanel webview tab shortcuts resolve against the active route surface's BrowserPanel owner id; Command-style chrome chords use `mod` (Command on macOS, Control elsewhere), while intentionally Control-specific chords retain `ctrl`; layout and surface actions set `allowInEditable` so they still fire from Composer contenteditable and bottom-panel xterm textareas; physical key codes cover unstable modified characters (including `Backquote` for `` ` ``), and workspace terminal launch only intercepts when an active workspace path is available.
