...
The preload contract exposes list, get, cancel, and task-change events. It never exposes request URLs, headers, artifact paths, or retry inputs. Windows NSIS remains transported by `electron-updater`, whose external progress is projected into the same task lifecycle. macOS Sparkle updates do not use Download Center for update bytes.
