...
- `chat-stream-broker.ts`：拥有 Electron main process 的 long-lived chat stream transport；main process 对 server SSE 保持每个 chat session 一个上游 stream，并通过 renderer IPC events fan out 已接受的 AI SDK chunk frames；response start request 透传 Cradle-owned `runtimeSettings`，同时用 `@cradleapp/ai-sdk` 维护可恢复的当前消息状态，late subscriber 先接收一个完整 transient snapshot 再继续接收 live chunks，避免把有界 replay tail 误当成恢复权威。
- `chat-stream-broker.test.ts`：覆盖 desktop chat stream broker 的单上游 fanout、late-subscriber snapshot recovery、有界诊断 replay buffer、delta coalescing、per-WebContents subscriber lifecycle cleanup、passive stream final unsubscribe abort，以及 response stream sender unsubscribe retention。
...
- `data-backup.ts`：拥有 `.cradle-backup` 单文件导出/恢复状态机。操作先写入固定 bootstrap manifest 并触发 Desktop 重启，在 Server 停止后打包或替换完整 Cradle server data tree；archive 内含逐文件 SHA-256 manifest，恢复只接受安全的普通文件/目录 entry，校验通过后才原子切换 active root。恢复后的 Server health check 失败会自动切回旧 root，成功后旧数据保留为 `.before-restore-*` rollback copy。
- `native-services.ts`：拥有 main-process native IPC service 注册，包括 native dialog/path launch IPC、Cradle data backup/restore file picker 与 restart scheduling、Desktop CLI install IPC、desktop chat stream broker IPC、Claude / Codex 会话文件的只读本机采样、Mac Appshot Cradle-native capture orchestration、Cradle-native image asset projection、Appshot source-window target locking、Codex temp asset observe-only evidence collection, and Appshot parity probe orchestration.
...
The desktop build also runs `scripts/build-chronicle.mjs`. On macOS it compiles the repository-owned Rust Chronicle runtime in release mode, copies it to `resources/chronicle/<platform>-<arch>/cradle-chronicle`, packages that directory under `Contents/Resources/chronicle`, and injects the resolved absolute path into the Server as `CRADLE_CHRONICLE_PATH` when the runtime is available. If the runtime is missing, Desktop logs a warning and starts the Server without Chronicle. Non-macOS hosts emit only a resource marker because the native capture runtime is currently macOS-only.
