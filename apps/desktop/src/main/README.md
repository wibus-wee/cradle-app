...
- `desktop-cli-manager.ts`：拥有 packaged Desktop CLI command lifecycle 的平台分发与共享 status contract；macOS（`desktop-cli-command-mac.ts`）读取 bundled launcher、检查 `/usr/local/bin/cradle` symlink 状态，Windows（`desktop-cli-command-windows.ts`）在 `%LOCALAPPDATA%\Cradle\bin` 写入 shim 并维护 HKCU user PATH entry，均通过显式用户操作安装、修复或移除 PATH command。macOS 权限提升只用于 symlink 写入；两个平台都不修改 shell rc 文件。
...
Desktop does not bundle or launch the Chronicle native runtime. Chronicle remains a Server-owned optional capability and may be supplied by a future independent deployment.
