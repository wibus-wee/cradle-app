...
- **sources/javascript.ts**: `javascript` source. Stores an Agent-authored JavaScript cell in the await filter, re-evaluates it each poll cycle inside the `javascript-eval` managed-process boundary, and maps the cell result contract to await outcomes. Opts into `resumeOnFailure`.
...
Evaluation failures are classified differently from invalid results. A syntax or module-shape problem is a permanent program error. A thrown cell, wall-clock timeout, or evaluator crash counts as a transient evaluation error and increments the row's `consecutive_error_count` (reset to 0 by any clean evaluation); the fifth consecutive evaluation error records count 5 and fails the await. The source evaluates at most three cells concurrently. `resumeText` and serialized `payload` are each capped at 32 KiB. The `javascript` source sets `resumeOnFailure`, so terminal failures also enqueue a failure-context resume message only when the pending-to-failed transition succeeds; cancellation or another trigger winning the race does not enqueue it.
