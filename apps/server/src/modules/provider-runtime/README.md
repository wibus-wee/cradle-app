...
- `host-manager.ts` owns provider-neutral host lease accounting and live resource lifetime. It tracks host identity, ref count, pinned lease count, TTL refresh, release, idle reaping, and resource disposal. Releasing the final lease leaves a resource available until its TTL expires, so a later request can reuse a warm host; a zero TTL opts into immediate disposal. Provider adapters still own native protocol semantics, but host resources such as app-server clients are acquired and released through this manager.
