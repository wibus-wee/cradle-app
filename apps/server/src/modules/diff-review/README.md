...
Commit plan generation is started through the agent-fix start/rerun routes with `expectedOutput: "commit"`. Those routes accept `outputLocale` for plan titles and rationale; commit message subjects still follow the repository's existing commit style rather than UI localization.
