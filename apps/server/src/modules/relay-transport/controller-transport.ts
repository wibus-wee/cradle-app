...
/**
 * Controller-side Fabric link transport.
 *
 * Connects to relayd `/v1/ws/controllers/:linkId` as one end of an
 * E2E-encrypted tunnel, runs the `FabricSession` handshake, and listens on a
 * local TCP port. Each TCP connection accepted on the local port becomes one
 * multiplexed stream over the tunnel: bytes flow local-socket → stream_data →
 * Node peer → Node's local Cradle Server socket, and back.
 *
 * The controller's X25519 private key is supplied by the Fabric membership
 * owner and is never generated by this transport.
 */

export interface RelayControllerTransportOptions {
  relayUrl: string
  /** Controller encryption key from the Fabric membership certificate. */
  controllerPrivateKeyBase64: string
  controllerPublicKeyBase64: string
  /** Node encryption key from the Fabric link certificate. */
  nodePublicKeyBase64: string
  readyTimeoutMs?: number
  /** Fabric v3 route and authenticated WebSocket headers. */
  fabric: FabricEnvelopeRoute & { headers: Headers }
}

export interface RelayControllerTransportHandle extends LocalTunnelHandle {
  /** The controller keypair used by this Fabric membership. */
  readonly controllerPrivateKeyBase64: string
  readonly controllerPublicKeyBase64: string
  /** The Node public key learned during the Fabric session handshake. */
  readonly nodePublicKeyBase64: string | null
  /**
...
class ControllerTransport {
  ...
  private nodePublicKeyBase64: string | null = null
  ...
  async start(readyTimeoutMs: number): Promise<void> {
    ...
    // Node peer may not be connected yet (its connector is reconnecting), in
  ...
  private connectAndHandshake(remainingMs: number): Promise<void> {
    ...
    const wsUrl = toWebSocketUrl(this.options.relayUrl, `/v1/ws/controllers/${encodeURIComponent(this.options.fabric.linkId)}`)
    return new Promise<void>((resolve, reject) => {
      ...
      let ws: WebSocket
      try {
        ws = new WebSocket(wsUrl, { headers: fabricHeadersRecord(this.options.fabric.headers) })
      }
 catch (error) {
      ...
      const session = new FabricSession(
        ...
        {
          fabricId: this.options.fabric.fabricId,
          linkId: this.options.fabric.linkId,
          ourPublicKeyBase64: this.keypair.publicKeyBase64,
          expectedPeerPubkey: this.options.nodePublicKeyBase64,
          encodeOutboundEnvelope: frame => encodeFabricEnvelope(this.options.fabric, frame),
        },
        {
          send: (data) => {
          ...
          onPeerPubkey: (pubkey) => {
            this.nodePublicKeyBase64 = pubkey
          },
  ...
  toHandle(): RelayControllerTransportHandle {
    ...
    return {
      nodeId: this.options.fabric.nodeId,
      localPort,
      ...
      nodePublicKeyBase64: this.nodePublicKeyBase64,
