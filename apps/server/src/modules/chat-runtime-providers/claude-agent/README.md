...
Stopping a current turn calls `interrupt()`. LinkCode's cancel path: if the interrupt control ack arrives **before** the cancelled turn's terminal `result`/EOF, Cradle **detaches** that live Query and the next prompt rebuilds from the last provider session id — so late cancellation fallout cannot settle (or zombie) the next turn. If a terminal `result`/EOF settles **while** awaiting interrupt, the Query stays and is ready for the next push. If `interrupt()` itself rejects, Cradle also detaches. UUIDs reported in `still_queued` are logged as facts; after detach, only inputs still observed as native `queued` remain safe for drain to resubmit on a replacement Query. Cancelling one submitted row calls `cancelAsyncMessage(uuid)` first; Cradle cancels the durable row only when Claude returns `true`.
