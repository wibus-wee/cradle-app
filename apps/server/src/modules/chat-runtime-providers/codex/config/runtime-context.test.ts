...
describe('codex runtime context native skill projection', () => {
  afterEach(() => {
  ...
  it('projects plugin skills into the agent native skill root and .agents compatibility path', () => {
    tempHome = fs.mkdtempSync(join(tmpdir(), 'cradle-codex-runtime-context-home-'))
  ...
  it('does not project into the global Codex skill root without the feature flag', () => {
    tempHome = fs.mkdtempSync(join(tmpdir(), 'cradle-codex-global-context-default-home-'))
    const previousHome = process.env.HOME
  ...
  it('projects plugin and builtin skills into the global Codex skill root when enabled', async () => {
    tempHome = fs.mkdtempSync(join(tmpdir(), 'cradle-codex-global-context-home-'))
    const previousHome = process.env.HOME
    const previousDataDir = process.env.CRADLE_DATA_DIR
