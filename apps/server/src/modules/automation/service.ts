...
import { automationArtifacts, automationDefinitions, automationEvents, automationRuns } from '@cradle/db'
...
function toDefinitionView(row: AutomationDefinition): AutomationDefinitionView {
  return {
    ...
    enabled: row.enabled,
    trigger: readJson<AutomationTrigger>(row.triggerJson, { type: 'rrule', rrule: '', timezone: 'UTC' }),
...
function toRunView(row: AutomationRun): AutomationRunView {
  return {
    ...
    status: row.status,
    triggerSnapshot: readJson<AutomationTrigger>(row.triggerSnapshotJson, { type: 'rrule', rrule: '', timezone: 'UTC' }),
...
function getDefinitionRow(id: string): AutomationDefinition {
  const row = db().select().from(automationDefinitions).where(eq(automationDefinitions.id, id)).get()
  if (!row) {
    throw new AppError({ code: 'automation_not_found', status: 404, message: 'Automation not found', details: { id } })
  }
  return row
}
...
function getRunRow(runId: string, definitionId?: string): AutomationRun {
  ...
  const row = db().select().from(automationRuns).where(and(...predicates)).get()
  if (!row) {
    throw new AppError({ code: 'automation_run_not_found', status: 404, message: 'Automation run not found', details: { runId } })
  }
  return row
}
...
function writeEvent(input: {
  ...
  db().insert(automationEvents).values({
    id: randomUUID(),
...
function buildRunPrompt(recipe: AutomationRecipe): string {
  const inputs = recipe.inputs.length > 0
    ...
    : ''
  const artifacts = recipe.artifactRequests.length > 0
...
export function list(input: { workspaceId?: string, enabled?: boolean }): AutomationDefinitionView[] {
...
export function setEnabled(id: string, enabled: boolean): AutomationDefinitionView {
  const existing = getDefinitionRow(id)
  const now = currentUnixSeconds()
  const trigger = readJson<AutomationTrigger>(existing.triggerJson, { type: 'rrule', rrule: '', timezone: 'UTC' })
  db().update(automationDefinitions).set({
    enabled,
    nextRunAt: enabled ? getNextOccurrence(trigger, now) : null,
    updatedAt: now,
  }).where(eq(automationDefinitions.id, id)).run()
  writeEvent({
    definitionId: id,
...
function insertRun(input: {
  ...
  const now = currentUnixSeconds()
  return db().insert(automationRuns).values({
    id: randomUUID(),
...
export function enqueueDueRuns(input: { now?: number, lookbackSeconds?: number, limit?: number } = {}): AutomationRunView[] {
  ...
  for (const definition of dueDefinitions) {
    const trigger = readJson<AutomationTrigger>(definition.triggerJson, { type: 'rrule', rrule: '', timezone: 'UTC' })
    const windowStart = definition.nextRunAt
      ?? Math.max(0, (definition.lastRunAt ?? now) - lookbackSeconds)
...
function insertScheduledRunIfMissing(definition: AutomationDefinition, occurrence: DueOccurrence): AutomationRun | null {
  try {
    return insertRun({
      definition,
...
export async function runNow(id: string, input: { occurrenceKey?: string, scheduledFor?: number } = {}): Promise<AutomationRunView> {
...
export async function executeRun(runId: string): Promise<AutomationRunView> {
  ...
  try {
    ...
    }).where(eq(automationRuns.id, run.id)).run()
    ...
    db().update(automationDefinitions).set({
      ...
      updatedAt: finishedAt,
    }).where(eq(automationDefinitions.id, run.automationDefinitionId)).run()
    writeEvent({ definitionId: run.automationDefinitionId, runId: run.id, type: 'automation.run.completed', message: 'Automation run completed' })
  }
  catch (error) {
...
export function getArtifact(definitionId: string, artifactId: string): AutomationArtifactView {
  ...
  const row = db()
    ...
    .get()
  if (!row) {
    throw new AppError({ code: 'automation_artifact_not_found', status: 404, message: 'Automation artifact not found', details: { artifactId } })
  }
