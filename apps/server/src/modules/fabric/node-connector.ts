...
import { assertFabricCertificate, fabricAuthHeaders, type MembershipCertificate } from './protocol'
import { getFabricMembership } from './service'
import { RelaySession } from '../relay-transport/session'
import { relayWebSocketDataView } from '../relay-transport/websocket-data'
...
export class FabricNodeConnector {
  ...
  private async connect(): Promise<void> {
    ...
    const headers = fabricAuthHeaders(membership.nodeCertificate, readSecret(membership.identityKeySecretId), 'GET', '/v1/ws/nodes')
    ...
    this.ws = ws
    ws.on('message', data => {
      try { this.handleEnvelope(relayWebSocketDataView(data), membership.nodeCertificate, readSecret(membership.encryptionKeySecretId), membership.fabricId) }
  ...
  private openLink(linkId: string, payload: Uint8Array, nodeCertificate: MembershipCertificate, encryptionPrivateKey: string, fabricId: string): void {
    ...
    if (controller.subjectKind !== 'controller' || controller.nodeId && controller.nodeId !== nodeCertificate.subjectId || !controller.scopes.some(scope => scope === 'control' || scope === 'admin')) throw new AppError({ code: 'fabric_controller_unauthorized', status: 403, message: 'Controller certificate cannot control this Node.' })
    ...
    const session = new RelaySession('host', encryptionPrivateKey, {
      roomId: linkId, ourPublicKeyBase64: nodeCertificate.encryptionPubkey, pairingCode: fabricId,
      ...
      onPeerClosed: () => this.closeLink(linkId), onError: () => this.closeLink(linkId),
      onPauseStream: streamId => streams.get(streamId)?.socket.pause(), onResumeStream: streamId => streams.get(streamId)?.socket.resume(),
    })
...
function readFabricNodeAuthToken(): string {
  ...
  try { return readSecret(id) } catch { const token = randomBytes(32).toString('base64url'); upsertSecret({ id, kind: 'system-fabric-node-auth-token', label: 'Cradle Fabric Node tunnel token', secret: token }); return token }
}

export function listActiveFabricNodeAuthTokens(): string[] {
  try { return [readFabricNodeAuthToken()] } catch { return [] }
}
...
class FabricHttpRequestWriter {
  ...
  private writeSocket(data: Buffer, original: number, consumed: (bytes: number) => void): void { this.socket.write(data, error => { if (error) this.socket.destroy(); else consumed(original) }) }
}
