...
import type { RelayEnvelope } from '../relay-transport/protocol'
import { RELAY_ENVELOPE_KIND, RELAY_PROTOCOL_VERSION, type RelayEnvelopeKind, type RelayPriority } from '../relay-transport/protocol'
import type { RelayOutboundEnvelope } from '../relay-transport/session'
...
function decode(value: Uint8Array, label: string): string { try { return new TextDecoder('utf-8', { fatal: true }).decode(value) } catch { throw protocolError(`${label} is invalid.`) } }
