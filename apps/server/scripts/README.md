...
- `rebuild-electron-runtime.mjs`: materializes native packages out of pnpm links, then force-rebuilds the server desktop runtime native dependencies in `dist/desktop-runtime/node_modules` from source against the Electron Node ABI used by Cradle desktop without mutating the workspace binary.
