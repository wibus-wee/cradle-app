...
try {
  ...
  const markdown = [
    ...
    `- Conversation pattern: \`${result.config.conversationPattern.kind}\``,
    `- Follow-up turns: ${result.counters.chatTurnsAccepted.toLocaleString()} every ${(result.config.conversationPattern.followUpIntervalMs / 1_000).toFixed(1)} seconds`,
    `- Context envelope: ${result.counters.chatContextTokens.toLocaleString()} approximate tokens / ${mib(result.counters.chatRequestBytes)} MiB largest JSON request`,
    `- Cumulative Chat request JSON: ${mib(result.counters.chatRequestBytesTotal)} MiB`,
    ...
    `- Renderer baseline working set: ${mib(result.summary.baselineRendererWorkingSetBytes)} MiB`,
    `- Renderer peak working set: ${mib(result.summary.peakRendererWorkingSetBytes)} MiB`,
    `- Renderer peak delta: ${mib(result.summary.rendererPeakDeltaBytes)} MiB`,
    `- Renderer settled delta: ${mib(result.summary.rendererSettledDeltaBytes)} MiB`,
