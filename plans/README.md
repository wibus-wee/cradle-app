...
| 018  | Code-split heavy panels and routes                       | P2       | M      | —          | TODO (partial: settings sections + ChatView/terminal panels lazy; remaining: global xterm CSS in `styles.css`, static monaco in `workspace-file-editor.tsx`, static `DiffHomePage` in `routes/diff.tsx`) |
...
| 029b | Review Guide UX overhaul (`029-review-guide-ux-overhaul`) | P2       | M      | —          | REJECTED (moot: implemented in `567b8de3`, then the guides feature was deleted in `05a222a0` — target code no longer exists) |
...
| 062  | Cradle Recall — agent cognition stack + CodeAct retrieval contract | P1 | XL     | 024, 041   | DONE (`modules/recall` query/attune services + rebuildable evidence projection; `recall_query`/`recall_attune` agent tools; `recall_attunements`/`recall_attunement_requests` tables; `specs/capabilities/recall.md`) |
