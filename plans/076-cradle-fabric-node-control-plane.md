...
- [x] (2026-08-16) Add relayd's durable Fabric directory, membership certificates, node presence, and authenticated directory API. Implemented SQLite store, signed join requests, owner-approved certificates, grants, snapshot-first directory routes, and revocation regression coverage; `go test ./...` passes in `apps/relayd`.
- [ ] Replace one-room/one-controller relay transport with one long-lived Node connection and many independently encrypted controller links. (implemented: relayd v3 link lifecycle, short-lived link expiry, Node-side controller-certificate delivery, TypeScript v3 envelope adapter, demand-driven controller link, and Node socket demultiplexer; remaining: real two-process transport acceptance test and removal of the room implementation.)
- [ ] Replace local `remote_hosts` and relay enrollments with one local Fabric membership and a Node directory cache. (implemented: local membership/secret references, create/join/approve directory client, and Node list endpoint; remaining: generated migration and legacy table removal.)
- [ ] Route workspace, session, chat, terminal, and provider requests through `nodeId`; retain target-Node authority and local session projection semantics. (implemented: renamed `remote-projection.ts` to `node-projection.ts`, switched the projection table and session execution contract to `node_session_links` / `execution.kind: 'node'`, and routed linked session HTTP through the demand-driven Fabric link; remaining: workspace locator and non-chat capability migration.)
- [ ] Replace Remote Hosts settings and workspace import surfaces with the Nodes product surface and automatic node enrollment flow.
- [ ] Delete all legacy remote-host, per-room pairing, SSH, direct URL, and relay-server-registry runtime paths; execute one-time legacy cleanup/export.
- [ ] Run focused transport, server, web, and two-node end-to-end acceptance tests; update this plan and `plans/README.md` with the outcome.
...
`POST /v1/fabrics` accepts an owner public key and a self-signed creation request, creates a random Fabric id, and returns no secret. An unenrolled Node creates a self-signed `POST /v1/join-requests` request containing its identity/encryption public keys plus a hash of a locally generated delivery secret, then renders `{fabricId, requestId, raw delivery secret}` as its short-lived QR-compatible join string. An owner Controller scans it, signs a membership certificate locally, and sends that exact certificate to `POST /v1/join-requests/{requestId}/approve`; relayd verifies and persists it but never signs membership on the owner's behalf. The Node polls `GET /v1/join-requests/{requestId}` with the secret and receives the same approved certificate idempotently after a lost response. Adding a Controller follows the same certificate/grant mechanism. A Controller opens a link only with a valid unrevoked `control`, `approve`, or `admin` grant. `GET /nodes` returns a compact `NodeSummary` filtered to grants; the events endpoint publishes `node.upsert`, `node.presence`, and `node.removed` revisions without workspace contents.
...
Refactor `apps/server/src/modules/relay-transport/session.ts`, `host-connector.ts`, and `controller-transport.ts` into `apps/server/src/modules/fabric/transport/`. A `FabricNodeConnector` owns one outbound authenticated Node WebSocket and `Map<linkId, RelaySession>`. A `FabricControllerLink` owns one Controller WebSocket and one `RelaySession`. Reuse the existing X25519 key agreement, HKDF derivation, adaptive XChaCha20-Poly1305/AES-256-GCM encryption, Zstandard behavior, 64 KiB chunks, and bounded credits. Do not replace a working encrypted transport with HTTP polling or a fresh TCP protocol.
