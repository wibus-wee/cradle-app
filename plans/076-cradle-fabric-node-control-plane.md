...
- **Execution**: DONE (two-Node e2e acceptance passed 2026-09-17 — see Progress)
- **Priority**: P0
- **Effort**: XL
...
- [x] (2026-09-17) Real two-Node end-to-end acceptance passed: `pnpm e2e:fabric` → CRADLE-FABRIC-001 green on current tree (1.2 min). The spec drives two real server processes + one real relayd + real Chromium Nodes UI through every manual-gate step: UI pairing with approval, remote Workspace mount, bidirectional remote chat, Node-owned Work creation + continuation, remote tool approval, relayd restart reconnect, target-Node restart recovery, and re-enrollment/stale-device removal. This satisfies the manual two-host smoke; a packaged-desktop-specific repeat is not separately gated by this plan.
