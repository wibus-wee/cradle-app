...
| Claude Agent × Cradle CLI × personal Plugin build/install × chat review × failed/successful update × Web contribution × reload | `CRADLE-PLUGIN-002` | A real Agent command builds and packs an immutable snapshot, binds checksum-specific permission review to the originating Chat, preserves the active revision after a failed build, revokes old trust after replacement, and loads the approved revision across reload |
