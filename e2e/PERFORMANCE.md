...
run on `cd648a44` then passed 33/33 interactions: 17/17 Web and 16/16 Mobile
samples from 5/5 Maestro flows, with no failed or missing response/boundary
metadata. Combined P50/P95/max were `3.084 s` / `37.019 s` / `70.880 s`; the
Mobile-only values were `7.363 s` / `70.880 s` / `70.880 s`. Against the earlier
hosted 33-sample run that later failed teardown, combined values changed from
`4.278 s` / `33.550 s` / `100.531 s` (`-27.91%` / `+10.34%` / `-29.49%`) and
Mobile-only values changed from `10.921 s` / `100.531 s` / `100.531 s`
(`-32.58%` / `-29.49%` / `-29.49%`). These mixed deltas remain runner variance.
The strengthened Settings boundary passed on its first attempt at `6.263 s`;
the target Node and every following interaction also passed. The pre-change
false-positive `7.617 s` is retained as failure evidence, not a valid hosted
latency baseline.
