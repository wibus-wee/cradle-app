...
功能: 精华聊天旅程
  ...
  @essence @P0 @CRADLE-CHAT-014
  场景: 应用进程重启会终结活动流并自动执行持久化跟进
    假如 我已配置可持久化队列的慢速 Claude Agent Simulator
    而且 我已添加了一个工作区
    而且 我已导航到新建聊天并选中 Simulator
    当 我在新建聊天输入框中输入"进程重启前的慢速任务"
    而且 我点击发送按钮
    那么 应该跳转到聊天视图
    而且 聊天流应处于进行中
    当 我在聊天输入框中输入"这是需要持久化的跟进"
    而且 我点击聊天发送按钮
    那么 跟进消息"这是需要持久化的跟进"应显示在聊天队列中
    当 Cradle Server 在活动流中崩溃并使用原数据目录重启
    那么 我应该看到用户消息"进程重启前的慢速任务"
    而且 我应该看到用户消息"这是需要持久化的跟进"
    而且 最后一条 AI 消息应包含"持久化队列跟进已完成"
    而且 聊天队列中不应显示跟进消息"这是需要持久化的跟进"
    而且 聊天流应结束于空闲状态
    而且 Simulator 脚本化交换应全部耗尽
